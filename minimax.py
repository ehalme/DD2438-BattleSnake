...
def start_minimax(game_state: typing.Dict, heuristic: Heuristic, max_depth: int, max_health: int, hazard_decay: int, step_decay: int) -> Action:
    ...
    for depth in range(1,max_depth+1):
        for actions in action_combos:
            ...
            score = minimax(new_board, my_snake, heuristic, calculation_time, start_time, depth, False, -math.inf, math.inf, transposition_table)
...
def minimax(board: Board, my_snake: str, heuristic: Heuristic, calculation_time: int, start_time: int, depth: int, maximizing: bool, alpha: int, beta: int, transposition_table: typing.Dict) -> float: # TODO: edit object
    ...
    if maximizing:
        ...
        for actions in action_combos:
            ...
            new_board.move_snakes(actions)
            score = max(score, minimax(new_board, my_snake, heuristic, calculation_time, start_time, depth - 1, False, alpha, beta, transposition_table))
        ...
        for actions in action_combos:
            ...
            new_board.move_snakes(actions)
            score = min(score, minimax(new_board, my_snake, heuristic, calculation_time, start_time, depth - 1, True, alpha, beta, transposition_table))
            beta = min(beta, score)
            if beta <= alpha:
                transposition_table[observation] = score
                break
